...
const SettingsScreen = ({ navigation }) => {
    ...
    return (
        <Box flex={1} _light={{bg:"#F1F3FD"}} _dark={{bg:"#393A45"}}>
            <ListItem title="設定"/>
            <Divider _light={{bg:"#41367C"}} _dark={{bg:"#F6F6F6"}} width={320} marginLeft={5}/>
            <HStack space={8} alignItems="center" justifyContent="space-between" >
                <Text fontSize={16} _light={{color:"#41367C"}} _dark={{color:"#F6F6F6"}} marginLeft={6}>{colorMode == "light" ? "淺色模式" : "深色模式"}</Text>
                <Switch
                    offThumbColor="#393A45" offTrackColor="white"
                    onThumbColor="#41367C" onTrackColor="white" 
                    marginRight={6}
                    name="light Mode"
            ...
            <Text fontSize={14} _light={{color:"#41367C"}} opacity={0.6} marginLeft={6} marginTop={-3}>0.00.02</Text>
